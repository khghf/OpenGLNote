...
namespace  mirror::Serialization
{
	...
	inline void SerializeJSonDefault(rapidjson::Value& jsonVal, const void* data, mirror::TypeId type, RapidJsonAllocator& allocator)
	{
		auto& info = type.GetInfo();
		auto& members = info.Members;
		/*{
			rapidjson::Value MemberVal{ rapidjson::kObjectType };
			MemberVal.SetString(info.Name.c_str(), allocator);
			jsonVal.AddMember(rapidjson::GenericStringRef{"TypeName"}, MemberVal, allocator);
		}*/
		for (auto& member : members)
		{
			if (!member.Variable.IsRefOrPointer())
			{
				...
				memberInfo.JSonSerializer(MemberVal, util::VoidOffset(data, member.Offset), allocator);
	...
	inline void DeserializeJSonDefault(rapidjson::Value& jsonVal, void* data, mirror::TypeId type)
	{
		auto& info = type.GetInfo();
		auto& members = info.Members;
		...
		for (auto& member : members)
		{
			if (!member.Variable.IsRefOrPointer())
			{
				auto memberVal = jsonVal.FindMember(member.Name.c_str());

				if (memberVal != jsonVal.MemberEnd())
				{
					auto& memberInfo = member.Variable.GetTypeId().GetInfo();
					memberInfo.JSonDeserializer(memberVal->value, util::VoidOffset(data, member.Offset));
				}
			}
		}
	}
	...
	inline void JSonSerializer<unsigned int>::Deserialize(rapidjson::Value& jsonVal, unsigned int& value)
	{
